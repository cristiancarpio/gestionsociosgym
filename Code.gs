...
function doPost(e) {
  try {
    ...
    if (action === 'sync_alerta')  return handleSyncAlerta(payload.data);
...
// ── ALERTAS ──────────────────────────────────────────────────
function handleSyncAlerta(data) {
  const headers = ['ID','Fecha','Nombre','Plan','Vencimiento','Leída'];
  const sheet = getOrCreateSheet(SHEET_ALERTAS, headers);
  const existing = sheet.getDataRange().getValues();
  const exists = existing.some(row => row[0] === data.id);
  if (!exists) sheet.appendRow([data.id, data.fecha, data.nombre, data.plan, data.vencimiento, 'no']);
  return resp({ ok: true });
}
...
function handleGetAll() {
  ...
  [SHEET_SOCIOS, SHEET_VENTAS, SHEET_ALERTAS].forEach(name => {
